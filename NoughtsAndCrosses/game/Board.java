...
public class Board {
	...
	public boolean hasPlayerWon(int player) {
		if ((board[0][0] == board[1][1] && board[0][0] == board[2][2] && board[0][0] == player) 			// check diagonals
			||
			...
			for (int i = 0; i<3; i++) {
				if ((board[i][0] == board[i][1] && board[i][0] == board[i][2] && board[i][0] == player)		// check rows
						||
						(board[0][i] == board[1][i] && board[0][i] == board[2][i] && board[0][i] == player)		// check columns
						) {
					return true;
				}
			}
